...
function defUser(e,n){return{email:e,name:n,balance:50,deposit:0,played:0,won:0,withdrawn:0,kyc:'pending',limits:{daily:500,weekly:2000,monthly:5000},selfExclusion:null,transactions:[{t:'bonus',a:50,d:'Bônus cadastro',dt:fmt()}],history:[],createdAt:new Date().toISOString()};}
...
function submitWithdraw(){
    ...
    state.user.transactions.unshift({t:'withdraw',a:-amount,d:'Saque PicPay R$ '+amount.toFixed(2),dt:fmt()});
    var all=getUsers();var idx=-1;for(var i=0;i<all.length;i++){if(all[i].email===state.user.email){idx=i;break;}}
