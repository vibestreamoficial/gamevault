...
function renderPage(p){
    switch(p){
        ...
        case 'admin':window.location.href='/panel';break;
...
function showModal(type){
    ...
    if(type==='login'){c.innerHTML=`<button class="modal-close" onclick="closeModal()"><i class="fas fa-times"></i></button><h2>Entrar</h2><div class="input-group"><label>Email</label><input type="email" id="loginEmail" placeholder="seu@email.com"></div><div class="input-group"><label>Senha</label><input type="password" id="loginPass" placeholder="••••••"></div><button class="btn btn-primary" onclick="doLogin()"><i class="fas fa-sign-in-alt"></i> Entrar</button><div class="alt-action">Não tem conta? <a href="#" onclick="showModal('register')">Cadastre-se</a></div><div style="text-align:center;margin-top:12px;"><a href="/panel" class="btn btn-outline btn-sm" style="text-decoration:none;font-size:11px;padding:6px 12px;"><i class="fas fa-user-shield"></i> Admin Panel</a> style="font-size:11px;padding:6px 12px;"><i class="fas fa-user-shield"></i> Admin</button></div>`;}
...
function showToast(msg,type='info'){let c=document.querySelector('.toast-container');if(!c){c=document.createElement('div');c.className='toast-container';document.body.appendChild(c);}const t=document.createElement('div');t.className=`toast toast-${type}`;t.textContent=msg;c.appendChild(t);setTimeout(()=>t.remove(),4000);}
...
// ===== REALITY CHECK =====
function checkReality(){if(!state.user)return;if((Date.now()-state.sessionStart)/60000>=60&&!state.realityShown){state.realityShown=true;const o=document.createElement('div');o.className='reality-check-overlay';o.innerHTML=`<div class="reality-check-box"><h2>⏰ Verificação</h2><p style="color:var(--text-secondary);">Jogando há ${Math.floor((Date.now()-state.sessionStart)/60000)} min</p><button class="btn btn-primary" onclick="this.closest('.reality-check-overlay').remove()">Continuar</button></div>`;document.body.appendChild(o);}}
